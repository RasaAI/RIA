...
export class MarketRiskComponent {
  ...
  // Send to your API
  this.http.post<any>('http://localhost:8000/upload-portfolio/', formData).subscribe({
    next: (res: any) => {
  ...
  toggleResult(): void {
    ...
    this.http.post<any>('http://localhost:8000/upload-multiple-csv/', formData).subscribe({
      next: (res: any) => {
